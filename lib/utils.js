...
function recomposeAuthority (component) {
  ...
  if (component.host !== undefined) {
    ...
    if (!isIPv4(host)) {
      ...
      if (ipV6res.isIPV6 !== true && ipV6res.isIPVFuture !== true) {
        // Decode percent escapes only when the raw host is not already a valid
        // IP literal, so a validated zone identifier is not decoded a second
        // time during recomposition.
        host = unescape(host)
  ...
  if (typeof component.port === 'number' || typeof component.port === 'string') {
    // RFC 3986 section 3.2.3 defines port as *DIGIT. Anything else can carry
    // authority delimiters ("@", "/", "?", "#", ":") that would demote the
    // host to userinfo or terminate the authority, so reject it instead of
    // recomposing an authority the caller never described.
